...
- **Gentoo** species has a high flipper length. There is slight positive skew in distribution as the median is closer to left.
- **Chinistrap** species has an average flipper length. The flipper length seems to have normal distrubution
- **Adelie** species has small flipper length. Similar to Gentoo species, they also have positive skew in distribution.
...
Boxplot helps to identify the distribution of the dataset. We can view the highest and the lowest values with median.

What about the jitterplot?

Jitterplot helps to identify the relationship between two or more variables.
...
Which is easier to read? Makrdown
Which looks more professional? Markdown
...
Which do you prefer? jitterplot over boxplot
...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm)) +
  geom_histogram(mapping = aes(color = sex))
```
...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm)) +
  geom_histogram(mapping = aes(color = island))
```
